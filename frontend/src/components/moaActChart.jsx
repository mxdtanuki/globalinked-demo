...
const MOAActivityChart = forwardRef(
  (
    {
      data,
      onDataUpdate,
    ...
    const months =
      selectedYear && data[selectedYear]
        ? MONTHS_ORDER.filter((m) =>
            Object.keys(data[selectedYear] || {}).includes(m),
          )
        : [];
    ...
    useEffect(() => {
      console.debug("MOAActivityChart effect", {
      ...
      if (data && Object.keys(data).length > 0) {
        ...
        const yearMonths = MONTHS_ORDER.filter((m) =>
          yearMonthsUnsorted.includes(m),
        );
        const useLatestMonth = yearMonths.length
          ? yearMonths[yearMonths.length - 1]
          : latestMonth || yearMonthsUnsorted.sort().reverse()[0];
        ...
        if (propSelectedYear && data[useYear] && yearMonths.length > 0) {
          ...
          console.debug("MOAActivityChart full-year", {
            useYear,
            monthsShown: yearMonths.length,
          ...
          const range =
            startIndex >= 0 && endIndex >= 0 && endIndex >= startIndex
              ? yearMonths.slice(startIndex, endIndex + 1)
              : [];
          const combined = {};
          range.forEach((m) => {
            const entries = data[useYear][m] || [];
          ...
          console.debug("MOAActivityChart combined range", {
            useYear,
