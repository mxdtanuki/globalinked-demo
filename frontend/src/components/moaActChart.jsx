...
const MOAActivityChart = forwardRef(({ data, onDataUpdate, selectedYear: propSelectedYear, visibleStartMonth, visibleEndMonth }, ref) => {
  ...
  // rely on the data-driven effect below; no mount-only initialization

  useEffect(() => {
    console.debug('MOAActivityChart effect', { dataKeys: Object.keys(data || {}), propSelectedYear, visibleStartMonth, visibleEndMonth });
  ...
  const useLatestMonth = yearMonths.length ? yearMonths[yearMonths.length - 1] : (latestMonth || yearMonthsUnsorted.sort().reverse()[0]);
      ...
      // If parent provided a selectedYear, combine all months present for that year
      if (propSelectedYear && data[useYear] && yearMonths.length > 0) {
        ...
        setRangeLabel(
          yearMonths.length === 1
            ? `${yearMonths[0]} ${useYear}`
            : `${yearMonths[0]} – ${yearMonths[yearMonths.length - 1]} ${useYear}`
        );
        console.debug('MOAActivityChart full-year', { useYear, monthsShown: yearMonths.length, resultCount: result.length });
        ...
        console.debug('MOAActivityChart combined range', { useYear, rangeLength: range.length, resultCount: result.length });
        ...
        for (const y of Object.keys(data)) {
          ...
          if (months.length > 0) {
            ...
            if (arr.length > 0) {
              ...
              // DO NOT call onDataUpdate automatically here; parent owns table state
